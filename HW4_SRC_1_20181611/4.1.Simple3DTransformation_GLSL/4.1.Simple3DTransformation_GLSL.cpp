...
void display(void) {
	...
	int cow_clock = (timestamp % 360) - 180;

	ModelViewProjectionMatrix = glm::rotate(ViewProjectionMatrix, cow_clock * TO_RADIAN, glm::vec3(1.0f, 1.0f, 0.0f));
	ModelViewProjectionMatrix = glm::translate(ViewProjectionMatrix,glm::vec3(sinf(cow_clock*TO_RADIAN),cow_clock/30.0, 0.0f));
	ModelViewProjectionMatrix = glm::rotate(ModelViewProjectionMatrix, 160.0f * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
...
void timer_scene(int timestamp_scene) {
	...
	rotation_angle_cow = (float)(timestamp_scene % 360 - 180);
	glutPostRedisplay();
