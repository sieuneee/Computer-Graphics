...
void display(void) {
	...
	int cow_clock = (timestamp % 360);
	for (int i = 0; i < 6; i++) {
		float len = (float)i;

		ModelViewProjectionMatrix = glm::rotate(ViewProjectionMatrix, cow_clock * TO_RADIAN, glm::vec3(0.0f, 1.0f, 0.0f));
		ModelViewProjectionMatrix = glm::translate(ModelViewProjectionMatrix, glm::vec3(len, 0.0f, 0.0f));
		...
		draw_axes();
		glLineWidth(1.0f);		
		draw_object(OBJECT_COW, 232 / 255.0f, 173 / 255.0f, 170 / 255.0f); // Rose
	}
	glutSwapBuffers();
}
