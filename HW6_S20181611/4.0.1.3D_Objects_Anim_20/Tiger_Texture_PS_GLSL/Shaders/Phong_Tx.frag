			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					if (!u_blind_effect) {
						...
						//tmp_float = cos(20*acos(tmp_float));
						tmp_float = tan(20*acos(tmp_float));
						if (tmp_float < 0.5) 
							tmp_float = zero_f;
...
void main(void) {
	...
	if (screen_effect) {
		...
		if(17*x_mod*x_mod-16*x_mod*y_mod+17*y_mod*y_mod<=2.25 || 17*x_mod*x_mod-16*(-x_mod)*y_mod+17*y_mod*y_mod<=2.25)
			discard;
