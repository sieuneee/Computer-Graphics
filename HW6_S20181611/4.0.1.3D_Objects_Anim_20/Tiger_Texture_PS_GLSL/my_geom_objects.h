...
void prepare_floor(void) { // Draw coordinate axes.
	...
	My_glTexImage2D_from_file("Data/static_objects/floor.jpg");

	glGenerateMipmap(GL_TEXTURE_2D);

	glActiveTexture(GL_TEXTURE0 + TEXTURE_ID_SOGANG);
	glBindTexture(GL_TEXTURE_2D, texture_names[TEXTURE_ID_SOGANG]);
	My_glTexImage2D_from_file("Data/static_objects/sogang.jpg");
